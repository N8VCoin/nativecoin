...
CzNATIVETracker::CzNATIVETracker(std::string strWalletFile)
{
...
CzNATIVETracker::~CzNATIVETracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzNATIVETracker::Init()
{
...
bool CzNATIVETracker::Archive(CMintMeta& meta)
{
...
bool CzNATIVETracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzNATIVETracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzNATIVETracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzNATIVETracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzNATIVETracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzNATIVETracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzNATIVETracker::GetSerialHashes()
{
...
CAmount CzNATIVETracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzNATIVETracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzNATIVETracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzNATIVETracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzNATIVETracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzNATIVETracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzNATIVETracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzNATIVETracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzNATIVETracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzNATIVETracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzNATIVETracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzNATIVEWallet* zNATIVEWallet)
{
    bool iszNATIVEWalletInitialized = (NULL != zNATIVEWallet);
    CMintMeta meta;
    ...
    if (! iszNATIVEWalletInitialized)
        zNATIVEWallet = new CzNATIVEWallet(strWalletFile);
    meta.isSeedCorrect = zNATIVEWallet->CheckSeed(dMint);
    if (! iszNATIVEWalletInitialized)
        delete zNATIVEWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzNATIVETracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzNATIVETracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzNATIVETracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzNATIVETracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzNATIVETracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzNATIVETracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzNATIVEWallet* zNATIVEWallet = new CzNATIVEWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            Add(dMint, false, false, zNATIVEWallet);
        }
        delete zNATIVEWallet;
        LogPrint("zero", "%s: added %d dzNATIVE from DB\n", __func__, listDeterministicDB.size());
    }

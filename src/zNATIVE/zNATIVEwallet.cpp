...
CzNATIVEWallet::CzNATIVEWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zNATIVE seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadzNATIVESeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.ErasezNATIVESeed_deprecated()) {
                    LogPrintf("%s: Updated zNATIVE seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zNATIVE seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zNATIVE wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzNATIVEWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WritezNATIVECount(nCountLastUsed);
    else if (!walletdb.ReadzNATIVECount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzNATIVEWallet::Lock()
{
    seedMaster = 0;
}

void CzNATIVEWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzNATIVEWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedTozNATIVE(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzNATIVEWallet::LoadMintPoolFromDB()
{
...
void CzNATIVEWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzNATIVEWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzNATIVEWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zNATIVETracker->HasPubcoinHash(pMint.first)) {
...
bool CzNATIVEWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedTozNATIVE(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zNATIVETracker which also adds to database
    pwalletMain->zNATIVETracker->Add(dMint, true);
...
void CzNATIVEWallet::SeedTozNATIVE(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzNATIVEWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzNATIVEWallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(strWalletFile);
    walletdb.WritezNATIVECount(nCountLastUsed);
...
void CzNATIVEWallet::GenerateDeterministiczNATIVE(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzNATIVEWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzNATIVEWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzNATIVEWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());

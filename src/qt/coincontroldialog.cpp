    ...
    if (ui->radioListMode->isChecked()) {
        ...
        for (int i = 0; i < ui->treeWidget->topLevelItemCount(); i++) {
            ...
            if (model->isLockedCoin(uint256(item->text(COLUMN_TXHASH).toStdString()), item->text(COLUMN_VOUT_INDEX).toUInt())) {
                ...
                item->setIcon(COLUMN_CHECKBOX, QIcon(QICON_PATH_COINLOCK));
            }
            updateLabelLocked();
        }
        ui->treeWidget->setEnabled(true);
...
void CoinControlDialog::lockCoin()
{
    ...
    contextMenuItem->setIcon(COLUMN_CHECKBOX, QIcon(QICON_PATH_COINLOCK));
    updateLabelLocked();
}
...
void CoinControlDialog::updateLabels(WalletModel* model, QDialog* dialog)
{
    ...
    int nDisplayUnit = BitcoinUnits::N8V;
    ...
    QString toolTip4 = tr("Can vary +/- %1 uN8V per input.").arg(dFeeVary);
        ...
        for(const COutput& out: coins.second) {
            ...
            if (fMultiSigUTXO) {
                ...
                if (!fMultisigEnabled) {
                    ...
                    itemOutput->setIcon(COLUMN_CHECKBOX, QIcon(QICON_PATH_COINLOCK));
                }
            } else {
            ...
            if (ExtractDestination(out.tx->vout[out.i].scriptPubKey, outputAddress)) {
                ...
                // if listMode or change => show nativecoin address. In tree mode, address is not shown again for direct wallet address outputs

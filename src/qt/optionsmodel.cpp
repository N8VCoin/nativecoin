...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::N8V);
    ...
    if (!settings.contains("nAnonymizenativecoinAmount"))
        settings.setValue("nAnonymizenativecoinAmount", 0);

    nAnonymizenativecoinAmount = settings.value("nAnonymizenativecoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("fEnableAutoConvert"))
        SoftSetBoolArg("-enableautoconvertaddress", false);
    if (settings.contains("nZeromintPercentage"))
        SoftSetArg("-zeromintpercentage", settings.value("nZeromintPercentage").toString().toStdString());
    if (settings.contains("nPreferredDenom"))
        SoftSetArg("-preferredDenom", settings.value("nPreferredDenom").toString().toStdString());
    if (settings.contains("nAnonymizenativecoinAmount"))
        SoftSetArg("-anonymizenativecoinamount", settings.value("nAnonymizenativecoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in nativecoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizenativecoinAmount:
            return QVariant(nAnonymizenativecoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizenativecoinAmount:
            nAnonymizenativecoinAmount = value.toInt();
            settings.setValue("nAnonymizenativecoinAmount", nAnonymizenativecoinAmount);
            emit anonymizenativecoinAmountChanged(nAnonymizenativecoinAmount);
            break;
        case CoinControlFeatures:

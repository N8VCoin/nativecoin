...
#include "ui_zNATIVEcontroldialog.h"
...
std::set<std::string> zNATIVEControlDialog::setSelectedMints;
std::set<CMintMeta> zNATIVEControlDialog::setMints;

bool CzNATIVEControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == zNATIVEControlDialog::COLUMN_DENOMINATION || column == zNATIVEControlDialog::COLUMN_VERSION || column == zNATIVEControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


zNATIVEControlDialog::zNATIVEControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::zNATIVEControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
zNATIVEControlDialog::~zNATIVEControlDialog()
{
    delete ui;
}

void zNATIVEControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void zNATIVEControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CzNATIVEControlWidgetItem* itemDenom(new CzNATIVEControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CzNATIVEControlWidgetItem *itemMint = new CzNATIVEControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zNATIVETracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zNATIVETracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zNATIVE.";
            else if (!mint.isSeedCorrect)
                strReason = "The zNATIVE seed used to mint this zNATIVE is not the same as currently hold in the wallet";
            else
...
void zNATIVEControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void zNATIVEControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelzNATIVE_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));
    //update PrivacyDialog labels
    privacyDialog->setzNATIVEControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> zNATIVEControlDialog::GetSelectedMints()
{
...
void zNATIVEControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);

...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.IsZerocoinSpend() && (fZSpendFromMe || wallet->zNATIVETracker->HasMintTx(hash))) {
            //zNATIVE stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakezNATIVE;
        ...
        for (const CTxOut& txout : wtx.vout) {
            ...
            if (txout.IsZerocoinMint()) {
                ...
                TransactionRecord sub(hash, nTime);
                sub.involvesWatchAddress = mine & ISMINE_WATCH_ONLY;
                sub.type = TransactionRecord::ZerocoinSpend_Change_zNATIVE;
                sub.address = mapValue["zerocoinmint"];
                if (!fFeeAssigned) {
        ...
        BOOST_FOREACH (const CTxOut& txout, wtx.vout) {
            ...
            if (mine) {
                ...
                if (ExtractDestination(txout.scriptPubKey, address) && IsMine(*wallet, address)) {
                    // Received by nativecoin Address
...
bool IszNATIVEType(TransactionRecord::Type type)
{
    switch (type) {
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakezNATIVE || type == TransactionRecord::MNReward) {

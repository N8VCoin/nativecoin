...
#include "zNATIVEcontroldialog.h"
#include "spork.h"
                                                          ...
                                                          fDenomsMinimized(true)
{
    ...
    // "Spending 999999 zNATIVE ought to be enough for anybody." - Bill Gates, 2017
    ui->zNATIVEpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzNATIVESyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zNATIVEpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzNATIVE_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zNATIVE is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zNATIVE, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zNATIVE...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zNATIVE in ") +
...
void PrivacyDialog::on_pushButtonSpendzNATIVE_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zNATIVE is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zNATIVE, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zNATIVE
        sendzNATIVE();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zNATIVE
    sendzNATIVE();
}

void PrivacyDialog::on_pushButtonzNATIVEControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    zNATIVEControlDialog* zNATIVEControl = new zNATIVEControlDialog(this);
    zNATIVEControl->setModel(walletModel);
    zNATIVEControl->exec();
}

void PrivacyDialog::setzNATIVEControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzNATIVESelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzNATIVE()
{
    QSettings settings;
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid nativecoin Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zNATIVEpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zNATIVE is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " N8V </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zNATIVE</b>";
    ...
    // use mints from zNATIVE selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!zNATIVEControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = zNATIVEControlDialog::GetSelectedMints();
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zNATIVE was spent successfully update the addressbook with the label
    ...
    // Clear zNATIVE selector in case it was used
    zNATIVEControlDialog::setSelectedMints.clear();
    ui->labelzNATIVESelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zNATIVE Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " N8V, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zNATIVE Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zNATIVETracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zNATIVE </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zNATIVE "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zNATIVE "));
    ui->labelzAvailableAmount_4->setText(QString::number(zerocoinBalance/COIN) + QString(" zNATIVE "));
    ui->labelzNATIVEAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zNATIVE </b> "));
    ui->labelZsupplyAmount_2->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zNATIVE </b> "));
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zNATIVE </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzNATIVESyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzNATIVE->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zNATIVE
        ui->pushButtonMintzNATIVE->setEnabled(false);
        ui->pushButtonMintzNATIVE->setToolTip(tr("zNATIVE is currently disabled due to maintenance."));

        // Spend zNATIVE
        ui->pushButtonSpendzNATIVE->setEnabled(false);
        ui->pushButtonSpendzNATIVE->setToolTip(tr("zNATIVE is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zNATIVE
        ui->pushButtonMintzNATIVE->setEnabled(true);
        ui->pushButtonMintzNATIVE->setToolTip(tr("PrivacyDialog", "Enter an amount of N8V to convert to zNATIVE", 0));

        // Spend zNATIVE
        ui->pushButtonSpendzNATIVE->setEnabled(true);
        ui->pushButtonSpendzNATIVE->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}

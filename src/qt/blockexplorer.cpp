...
static std::string ValueToString(CAmount nValue, bool AllowNegative = false)
{
    ...
    QString Str = BitcoinUnits::formatWithUnit(BitcoinUnits::N8V, nValue);
...
{
    std::string genesisblockhash = "00000b7892e23de3b819bb38f04cd3ff340e48ad7c379822a4ffa2b6a957eb9c";
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", true)) {
            QString Warning = tr("Not all transactions will be shown. To view all transactions you need to set txindex=1 in the configuration file (nativecoin.conf).");
            QMessageBox::warning(this, "NativeCoin Core Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}

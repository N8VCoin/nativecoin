...
public:
    ...
    bool WritezNATIVESeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadzNATIVESeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadzNATIVESeed_deprecated(uint256& seed);
    bool ErasezNATIVESeed();
    bool ErasezNATIVESeed_deprecated();

    bool WritezNATIVECount(const uint32_t& nCount);
    bool ReadzNATIVECount(uint32_t& nCount);

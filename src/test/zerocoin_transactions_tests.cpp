...
BOOST_AUTO_TEST_CASE(zerocoin_spend_test)
{
    ...
    cWallet.zNATIVETracker = unique_ptr<CzNATIVETracker>(new CzNATIVETracker(cWallet.strWalletFile));
    ...
    BOOST_CHECK_MESSAGE(receipt.GetStatus() == zNATIVE_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");
    ...
    BOOST_CHECK_MESSAGE(receipt2.GetStatus() == zNATIVE_TRX_FUNDS_PROBLEMS, "Failed Invalid Amount Check");

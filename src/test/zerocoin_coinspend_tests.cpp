...
BOOST_AUTO_TEST_CASE(zerocoin_wrapped_serial_spend_test)
{
    ...
    CzNATIVEWallet *czNATIVEWallet = new CzNATIVEWallet(wallet.strWalletFile);
    ...
    for (unsigned int i = 0; i < TESTS_COINS_TO_ACCUMULATE; i++) {
        ...
        czNATIVEWallet->GenerateDeterministiczNATIVE(denom, coin, dMint, true);
        czNATIVEWallet->UpdateCount();

...
#include "zNATIVE/accumulatormap.h"
#include "addrman.h"
#include "alert.h"
...
#if defined(NDEBUG)
#error "nativecoin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uN8V) are considered zero fee (for relaying and mining)
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zNATIVE spend with serial %s is already in block %d\n", __func__,
...
{
    //Check to see if the zNATIVE is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        try {
            if (!spend.HasValidSignature())
                return error("%s: V2 zNATIVE spend does not have a valid signature\n", __func__);
        ...
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zNATIVE without the correct spend type. txid=%s\n", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (!spend.HasValidSerial(Params().Zerocoin_Params(fUseV1Params))) {
        ...
        if (!isBlockBetweenFakeSerialAttackRange(pindex->nHeight))
            return error("%s : zNATIVE spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zNATIVE spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zNATIVE");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zNATIVE mints are not already known
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool iszNATIVEStake){
    int64_t ret = 0;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from nativecoin
...
{
    RenameThread("nativecoin-scriptch");
...
void RecalculatezNATIVEMinted()
{
    CBlockIndex *pindex = chainActive[Params().Zerocoin_StartHeight()];
    uiInterface.ShowProgress(_("Recalculating minted zNATIVE..."), 0);
    while (true) {
        ...
        if (pindex->nHeight % 1000 == 0) {
            ...
            uiInterface.ShowProgress(_("Recalculating minted zNATIVE..."), percent);
        }
...
void RecalculatezNATIVESpent()
{
    CBlockIndex* pindex = chainActive[Params().Zerocoin_StartHeight()];
    uiInterface.ShowProgress(_("Recalculating spent zNATIVE..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)(pindex->nHeight - Params().Zerocoin_StartHeight()) * 100 / (chainActive.Height() - Params().Zerocoin_StartHeight()))));
            uiInterface.ShowProgress(_("Recalculating spent zNATIVE..."), percent);
        }

        //Rewrite zNATIVE supply
        CBlock block;
        assert(ReadBlockFromDisk(block, pindex));
        ...
        //Remove spends from zNATIVE supply
...
bool RecalculateN8VSupply(int nHeightStart)
{
    ...
    uiInterface.ShowProgress(_("Recalculating N8V supply..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainActive.Height() - nHeightStart))));
            uiInterface.ShowProgress(_("Recalculating N8V supply..."), percent);
        }
        ...
        if(pindex->nHeight == 2195){
            //500k N8V Burnt in tx f9c851fddc613db8ce3469b88faa28c373ed7881a752d4aac28a0463cd42db8e
            pindex->nMoneySupply -= 500000 * COIN;

        }
        if(pindex->nHeight == 2216){
            //717,064.07 N8V Burnt in tx 243e04d31933a998462b4f13f013cfa97e994a8d82141c7a9ce1d6ff45d58588
...
bool UpdatezNATIVESupply(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    std::list<CZerocoinMint> listMints;
    ...
    // A one-time event where only the zNATIVE supply was off (due to serial duplication off-chain on main net)
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    bool fEnforceBIP30 = (!pindex->phashBlock) || // Enforce on CreateNewBlock invocations which don't have a hash.
                         !((pindex->nHeight == 0 && pindex->GetBlockHash() == uint256("00000b7892e23de3b819bb38f04cd3ff340e48ad7c379822a4ffa2b6a957eb9c")));
               //||
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        ...
        RecalculateN8VSupply(Params().Zerocoin_StartHeight());
    }

    //Track zNATIVE money supply in the block index
    if (!UpdatezNATIVESupply(block, pindex, fJustCheck))
        return state.DoS(100, error("%s: Failed to calculate new zNATIVE supply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zNATIVESpent: %s\n",
...
#ifdef ENABLE_WALLET
    // If turned on AutoZeromint will automatically convert N8V to zNATIVE
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zNATIVE spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn& txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zNATIVE serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zNATIVE stake block is less than allowed start height", __func__);

    if (CzNATIVEStake* zNATIVE = dynamic_cast<CzNATIVEStake*>(stake)) {
        CBlockIndex* pindexFrom = zNATIVE->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zNATIVE stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zNATIVE stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zNATIVE->GetValue()));
        if (nChecksum200 != zNATIVE->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zNATIVE->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IszNATIVE() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zNATIVE fails context checks", __func__));
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> n8vInputs;
        std::vector<CTxIn> zNATIVEInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.scriptSig.IsZerocoinSpend()){
                zNATIVEInputs.push_back(stakeIn);
            }else{
                n8vInputs.push_back(stakeIn);
            }
        }
        const bool hasN8VInputs = !n8vInputs.empty();
        const bool haszNATIVEInputs = !zNATIVEInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasN8VInputs)
                    ...
                    for (const CTxIn& N8VIn : n8vInputs){
                        if(N8VIn.prevout == in.prevout){
        ...
        if (isBlockFromFork) {
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction &t : bl.vtx) {
                    for (const CTxIn &in: t.vin) {
                        ...
                        for (const CTxIn &stakeIn : n8vInputs) {
            ...
            // Now that this loop if completed. Check if we have zNATIVE inputs.
            if(haszNATIVEInputs){
                for (const CTxIn& zNATIVEInput : zNATIVEInputs) {
                    CoinSpend spend = TxInToZerocoinSpend(zNATIVEInput);
                    ...
                    if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, spend, pindex, 0))
                        return state.DoS(100,error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                   stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zNATIVE");
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zNATIVE mints and %d zNATIVE spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // nativecoin: We use certain sporks during IBD, so check to see if they are
        ...
        if(nLocalServices & NODE_BLOOM_LIGHT_ZC) {
            try {
                ...
                if (!GetAccumulatorValue(height, den, bnAccValue)) {
                    LogPrint("zNATIVE", "peer misbehaving for request an invalid acc checkpoint \n", __func__);
                    Misbehaving(pfrom->GetId(), 50);
                } else {
            ...
            try {
                ...
                if (gen.isValid(chainActive.Height())) {
                    if (!lightWorker.addWitWork(gen)) {
                        LogPrint("zNATIVE", "%s : add genwit request failed \n", __func__);
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
